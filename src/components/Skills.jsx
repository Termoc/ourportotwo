...
const skills = [
  { name: "React", icon: <FaReact />, color: "text-cyan-400", bgColor: "bg-cyan-400/10", borderColor: "border-cyan-400/30" },
  { name: "Node.js", icon: <FaNode />, color: "text-green-400", bgColor: "bg-green-400/10", borderColor: "border-green-400/30" },
  { name: "Python", icon: <FaPython />, color: "text-yellow-400", bgColor: "bg-yellow-400/10", borderColor: "border-yellow-400/30" },
  { name: "SQL", icon: <SiMysql />, color: "text-orange-400", bgColor: "bg-orange-400/10", borderColor: "border-orange-400/30" },
];

const Skills = () => {
  return (
    <section id="skills" className="py-24 bg-black text-white px-6 relative overflow-hidden">
      {/* Background Effects */}
      <div className="absolute top-1/4 left-0 w-72 h-72 bg-blue-600/10 rounded-full blur-3xl"></div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {skills.map((skill, index) => (
            ...
            >
              ...
              {/* Corner accent */}
              <div className="absolute top-4 right-4 w-6 h-6 border-r-2 border-t-2 border-blue-400/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              
              <div className="relative z-10 flex flex-col items-center">
                {/* Icon Container - Fixed positioning */}
                <div className={`w-20 h-20 mb-6 rounded-2xl ${skill.bgColor} border ${skill.borderColor} flex items-center justify-center group-hover:scale-110 transition-all duration-500`}>
                ...
                <div className="w-full bg-gray-800/50 rounded-full h-1.5 mb-3">
                  <div 
                    className={`h-1.5 rounded-full bg-gradient-to-r from-${skill.color.replace('text-', '')} to-blue-400 transition-all duration-1000 group-hover:w-full`}
                    style={{ width: '85%' }}
                  ></div>
                </div>
              ...
              {/* Hover border glow */}
              <div className={`absolute inset-0 rounded-2xl border opacity-0 group-hover:opacity-100 transition-all duration-300 ${skill.borderColor.replace('/30', '/50')}`}></div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {['JavaScript', 'TypeScript', 'MongoDB', 'Express.js', 'Git', 'Docker', 'AWS', 'Tailwind CSS'].map((tech, index) => (
              <div 
                key={tech}
